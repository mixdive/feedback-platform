...
export default function MarkdownEditor({
    ...
    await Promise.all(
      files.map(async (file, i) => {
        ...
        try {
          ...
          const msg = err instanceof Error ? err.message : t('markdownEditor.uploadFailed')
          toast.error(msg)
          const current = latestRef.current
  ...
  const buttons = [
    { icon: Heading2, label: t('markdownEditor.heading'), run: () => prefixLines('## ') },
    { icon: Bold, label: t('markdownEditor.bold'), run: () => wrap('**', '**', 'bold') },
    { icon: Italic, label: t('markdownEditor.italic'), run: () => wrap('_', '_', 'italic') },
    { icon: Code, label: t('markdownEditor.code'), run: () => wrap('`', '`', 'code') },
    { icon: LinkIcon, label: t('markdownEditor.link'), run: insertLink },
    { icon: List, label: t('markdownEditor.bulletList'), run: () => prefixLines('- ') },
    { icon: ListOrdered, label: t('markdownEditor.numberedList'), run: () => prefixLines((i) => `${i + 1}. `) },
    { icon: Quote, label: t('markdownEditor.quote'), run: () => prefixLines('> ') },
  ]
      ...
      <div className="flex items-center justify-between gap-1 border-b border-zinc-200 px-2 py-1 dark:border-zinc-800">
        <div className="flex items-center gap-0.5">
          ...
          {upload && (
            <button
              type="button"
              title={t('markdownEditor.attachFile')}
        ...
        <div className="flex items-center gap-0.5">
          ...
          >
            <Pencil className="size-3.5" /> {t('markdownEditor.write')}
          </button>
          <button
            type="button"
          ...
          >
            <Eye className="size-3.5" /> {t('markdownEditor.preview')}
          </button>
        </div>
      </div>
      ...
      {mode === 'edit' ? (
        ...
        <div className="min-h-[120px] p-3">
          {value.trim() ? (
            ...
            <p className="text-sm text-zinc-400">{t('markdownEditor.nothingToPreview')}</p>
          )}
        </div>
      )}
    </div>
